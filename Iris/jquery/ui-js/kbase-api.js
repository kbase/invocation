    ...
    $.KBWidget({
        ...
        _textStyle: function() {
            ...
            var $prompt = $("<span></span>").append($("<a></a>").attr("id", "loginlink").attr("href", "#").text("Sign In").bind("click", $.proxy(function(e) {
                ...
                e.stopPropagation();
                this.openDialog();
            }, this))).append($("<div></div>").addClass("btn-group").attr("id", "userdisplay").css("display", "none").append($("<button></button>").addClass("btn btn-default").addClass("btn-xs").addClass("dropdown-toggle").append($("<span></span>").addClass("glyphicon glyphicon-user")).append($("<span></span>").addClass("caret")).bind("click", function(e) {
                e.preventDefault();
                e.stopPropagation();
                $(this).next().toggle();
            })).append($("<ul></ul>").addClass("dropdown-menu").addClass("pull-right").css("padding", "3px").attr("id", "login-dropdown-menu").append($("<li></li>").css("border-bottom", "1px solid lightgray").css("white-space", "nowrap").append($.jqElem("div").css("text-align", "right").append($("<a></a>").attr("id", "loggedinuser_id").css("font-weight", "bold").attr("href", "https://gologin.kbase.us/account/UpdateProfile").attr("target", "_blank").css("padding-right", "0px").css("padding-left", "0px")))).append($("<li></li>").addClass("pull-right").append($("<span></span>").append($("<a></a>").css("padding-right", "0px").css("padding-left", "0px").append("Sign out")).bind("click", $.proxy(function(e) {
                e.stopPropagation();
                e.preventDefault();
        ...
        _createLoginDialog: function() {
            ...
            var $ld = $("<div></div").kbasePrompt({
                ...
                body: $("<p></p>").append($("<form></form>").attr("name", "form").attr("id", "form").addClass("form-horizontal").append($("<fieldset></fieldset>").append($("<div></div>").attr("class", "alert alert-error").attr("id", "error").attr("style", "display : none").append($("<div></div>").append($("<div></div>").addClass("pull-left").append($("<i></i>").addClass("icon-warning-sign").attr("style", "float: left; margin-right: .3em;"))).append($("<div></div>").append($("<strong></strong>").append("Error:\n")).append($("<span></span>").attr("id", "errormsg"))))).append($("<div></div>").attr("class", "alert alert-success").attr("id", "pending").attr("style", "display : none").append($("<div></div>").append($("<div></div>").append($("<strong></strong>").append("Logging in as:\n")).append($("<span></span>").attr("id", "pendinguser"))))).append($("<div></div>").attr("class", "form-group").append($("<label></label>").addClass("control-label").addClass("col-lg-2").attr("for", "user_id").css("margin-right", "10px").append("Username:\n")).append($.jqElem("div").addClass("col-lg-9").append($("<input>").addClass("form-control").attr("type", "text").attr("name", "user_id").attr("id", "user_id").attr("size", "20")))).append($("<div></div>").attr("class", "form-group").append($("<label></label>").addClass("control-label").addClass("col-lg-2").attr("for", "password").css("margin-right", "10px").append("Password:\n")).append($.jqElem("div").addClass("col-lg-9").append($("<input>").addClass("form-control").attr("type", "password").attr("name", "password").attr("id", "password").attr("size", "20")))))),
                ...
                $.ajax({
                    ...
                    success: $.proxy(function(data, res, jqXHR) {
                        if (data.kbase_sessionid) {
                            if ($.cookie) {
                                $.cookie("kbase_session", "unEQUALSSIGN" + data.user_id + "PIPESIGN" + "kbase_sessionidEQUALSSIGN" + data.kbase_sessionid + "PIPESIGN" + "token_idEQUALSSIGN" + data.kbase_sessionid);
                            }
                            var cookieArray = [];
                            var args = {
                                success: 1
...
(function($, undefined) {
    $.KBWidget({
        ...
        init: function(options) {
            ...
            var container = $('<div class="panel panel-default">                                <div class="panel-heading">                                    <h4 class="panel-title"></h4>                                    <div class="panel-subtitle"></div>                                </div>                                <div class="panel-body"></div>                           </div>');
            ...
            this.loading = function() {
                panel_body.html('<p class="muted ajax-loader">                 <img src="assets/img/ajax-loader.gif"> loading...</p>');
            };
        ...
        render: function() {
            ...
            if (self.options.workspaceID === undefined) {
                ...
                if (wsid !== null && wsid[1] && wsid[2]) {
                    ...
                    self.trigger("error", [ "Cannot parse workspace ID " + self.options.workspaceID ]);
                    return self;
                }
            } else {
                fetchAjax = $.ajax({
            ...
            KBVis.require([ "jquery", "underscore", "renderers/network", "util/viewport", "text!sample-data/network1.json", "transformers/netindex", "util/slider", "text!templates/checkbox.html", "jquery-ui" ], function(JQ, _, Network, Viewport, Example, NetIndex, Slider, CheckboxTemplate) {
                ...
                var network = new Network({
                    ...
                    edgeFilter: function(edge) {
                        return edge.source != edge.target && (edge.strength >= minStrength || edge.source.type === "CLUSTER" || edge.target.type === "CLUSTER") && datasetFilter(edge);
                    },
                    nodeInfo: function(node, makeRow) {
                        ...
                        if (node.type === "GENE" && node.userAnnotations !== undefined) {
                            ...
                            if (annotations["external_id"] !== undefined) makeRow("External ID", link(annotations["external_id"], "#"));
                            if (annotations["functions"] !== undefined) makeRow("Function", annotations["functions"]);
                            if (annotations.ontologies !== undefined) {
                                var goList = JQ("<ul/>");
                                _.each(_.keys(annotations.ontologies), function(item) {
                                    goList.append(JQ("<li/>").append(link(item, goLink({
                                        id: item
                                    }))));
                                });
                    ...
                    searchTerms: function(node, indexMe) {
                        ...
                        if (node.userAnnotations !== undefined) {
                            var annotations = node.userAnnotations;
                            if (annotations["functions"] !== undefined) indexMe(annotations["functions"]);
                        }
                    }
                });
                ...
                forceSlider(viewport, "gravity", "Gravity", "G", .012);
                ...
                function addClusterDropdown($container, data) {
                    ...
                    var clusters = [];
                    _.map(_.filter(data.nodes, function(n) {
                    ...
                    _.each(_.sortBy(clusters, function(entry) {
                        ...
                        var labelDiv = JQ("<div>", {
                            ...
                            style: "float: left"
                        }).html(entry.node.entityId)).append(JQ("<span>", {
                    ...
                    list.find("input[type='checkbox']").click(function(event) {
                        ...
                        if (id === "all") {
                            list.find(clSelect).prop("checked", checked).trigger("change");
                        } else {
                            if (list.find(clSelect + ":not(:checked)").length === 0) allCheckbox.prop("checked", true); else allCheckbox.prop("checked", false);
                        }
                ...
                function unhideCluster(cluster) {
                    ...
                    _.each(neighbors, function(nPair) {
                        var node = nPair[0];
                        if (node.type === "GENE") network.unhideNode(node);
                    });
                }
                function hideCluster(cluster) {
                    ...
                    _.each(neighbors, function(nPair) {
                        var node = nPair[0];
                        if (node.type === "GENE") network.hideNode(node);
                    });
                ...
                function addDatasetDropdown($container, data) {
                    ...
                    list.find("a").on("click", function(event) {
                        ...
                        if (id == "all") datasetFilter = function() {
                            return true;
                        }; else datasetFilter = function(edge) {
            ...
            function formatObjs(objs, type) {
                ...
                if (type == "rxn") {
                    ...
                    for (var i in objs) {
                        ...
                        var compart = obj[0].split("_")[1];
                        obj[0] = '<a class="cpd-click" data-cpd="' + cpd + '">' + cpd + "</a> (" + compart + ")";
                        fluxes.push(obj);
                    }
                }
                return fluxes;
...
(function($, undefined) {
    $.KBWidget({
        ...
        render: function(options) {
            ...
            var jobsList = [];
            var data = {};
            jobsList.push(this.cdmiClient.fids_to_feature_data([ self.options.featureID ], function(featureData) {
                data.featureData = featureData[self.options.featureID];
            }, this.clientError));
            jobsList.push(this.cdmiClient.fids_to_genomes([ self.options.featureID ], function(genome) {
                data.genome = genome[self.options.featureID];
            }, this.clientError));
            jobsList.push(this.cdmiClient.fids_to_dna_sequences([ this.options.featureID ], function(dnaSeq) {
                data.dnaSeq = dnaSeq[self.options.featureID];
            }, this.clientError));
            jobsList.push(this.cdmiClient.fids_to_protein_families([ this.options.featureID ], function(families) {
            ...
            $.when.apply($, jobsList).done(function() {
                ...
                self.$table.append(self.makeRow("Genome", $("<div/>").append(data.featureData.genome_name).append("<br/>").append(self.makeGenomeButton(data.genome))));
                self.$table.append(self.makeRow("Length", data.featureData.feature_length + " bp"));
                self.$table.append(self.makeRow("Location", $("<div/>").append(self.parseLocation(data.featureData.feature_location)).append(self.makeContigButton(data.featureData.feature_location))));
                self.$table.append(self.makeRow("GC Content", self.calculateGCContent(data.dnaSeq).toFixed(2) + "%"));
                if (data.families && data.families.length != 0) {
                    self.cdmiClient.protein_families_to_functions(data.families, function(families) {
                        var familyStr = "";
                ...
                } else self.$table.append(self.makeRow("Protein Families", "None found"));
            });
        ...
        makeGenomeButton: function(genomeID, workspaceID) {
            ...
            var self = this;
            var $genomeBtn = $("<button />").addClass("btn btn-default").append("Show Genome").on("click", function(event) {
...
(function($, undefined) {
    $.KBWidget({
        ...
        render: function(options) {
            ...
            var self = this;
            this.$elem.append($("<button />").addClass("btn btn-primary").append("Show Description").on("click", function(event) {
                ...
                if (self.options.isInCard) {
                    self.cdmiClient.genomes_to_contigs([ self.options.genomeID ], function(contigs) {
                        self.cdmiClient.contigs_to_lengths(contigs[self.options.genomeID], function(contigsToLengths) {
                            ...
                            self.$elem.append($("<button class='btn btn-primary'>Show Contig</button>").on("click", function(event) {
                ...
                function showGapfillSolutions(tr, gapRef) {
                    ...
                    $.when(gapAJAX).done(function(data) {
                        ...
                        for (var i in sols) {
                            ...
                            var solID = sol.id;
                            if (sol.integrated == "1") {
                                solList.append('<div> <a type="button" class="gap-sol"                                data-toggle="collapse" data-target="#' + gapRef + solID.replace(/\./g, "_") + '" >' + solID + '</a> <span class="caret" style="vertical-align: middle;"></span>                                 </div>');
                            } else {
                                solList.append('<div> <a type="button" class="gap-sol"                                data-toggle="collapse" data-target="#' + gapRef + solID.replace(/\./g, "_") + '" >' + solID + '</a> <span class="caret" style="vertical-align: middle;"></span>                                </div>');
                            }
