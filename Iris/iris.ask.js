...
var Terminal = new Class(
{
	...
	p.grab(new Element('span').addClass('command').set('text', this.mode + ": " + this.cwd + "> " + text));
	...
	if (command == 'help' && this.mode == "iris") {
	...
	if (d = command.match(/^ls\s*(.*)/)) {
	    ...
	    var f = this.client.list_files_async(this.sessionId, this.cwd, d, 
			 function (filelist)
			 {
			     ...
			     {
				tbl += "<tr>";
				tbl += "<td>0</td><td>" + dirs[d]['mod_date'] + "</td>"; 
				tbl += "<td>" + dirs[d]['name'] + "</td>"; 
				tbl += "</tr>";
			     }
				...
				tbl += "<tr>";
				tbl += "<td>" + files[i]['size'] + "</td>"; 
				tbl += "<td>" + files[i]['mod_date'] + "</td>"; 
				tbl += "<td>" + "<a target='_blank' href='" + url + "'>" +
                                                files[i]['name'] + "</a>" + "</td>"; 
				tbl += "</tr>";
			     }
			     obj.out_to_div(commandDiv, tbl + "</table>");
			 }.bind(this),
			 function (err)
			 {
			     var m = err.message.replace("\n", "<br>\n");
	...
	if (this.mode == "iris") {
		...
		this.AskKB.askKB_async(this.sessionId, this.cwd, command, 
					       function (runout)
					       {
						   if (runout)
						   {
						       obj.out_to_div(commandDiv, runout);
						   }
						   else
						   {
						       obj.out_to_div(commandDiv, "Error running command.");
